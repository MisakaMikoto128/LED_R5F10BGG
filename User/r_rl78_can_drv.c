...
Can_RtnType R_CAN_Init(void)
{
    ...
    if ((GSTS & CAN_GLB_STP_STS_BIT_ON) != 0U)
    {
        ...
        GCTRL &= (uint16_t)~(CAN_GLB_STP_BIT_ON); //GSLPR set 0   reg:GCTRL
        if ((GSTS & CAN_GLB_STP_STS_BIT_ON) != 0U) 
        {
    ...
    if ((C0STSL & CAN_STP_STS_BIT_ON) != 0U)
    {
        ...
        C0CTRL &= (uint16_t)~(CAN_STP_BIT_ON);  //(Set the CSLPR bit in the CiCTRL register to 0
        if ((C0STSL & CAN_STP_STS_BIT_ON) != 0U)
        {
            return CAN_RTN_MODE_WAIT;
        }
    ...
    GCFGL = CAN_CFG_GLB_CFGL; //souce fclk(32M) 4fre = 8M
    GCFGH = CAN_CFG_GLB_CFGH; //no effect

    /* ==== communication speed setting ==== */
    C0CFGL = CAN_CFG_C0_BAUDRATE_L; //CAN_CFG_C0_BAUDRATE_L=1 BOUND/2
    C0CFGH = CAN_CFG_C0_BAUDRATE_H; //1BIT = 19TQ
...
#if CAN_RX_RULE_NUM > 0
    {
        ...
        GAFLCFG = CAN_CFG_RN0; // CAN_CFG_RN0=2  
    ...
    RMNB = CAN_CFG_RBNUM; //CAN_CFG_RBNUM = 1 

    /* ---- Set Rx FIFO buffer ---- */
    RFCC0 = CAN_CFG_RXFIFO0; //接收8条消息，关闭接收中断，接收中断源是FIFO缓存满，FIFO满了才中断 
    RFCC1 = CAN_CFG_RXFIFO1; //未启用

    /* ---- Set common (Tx/Rx) FIFO buffer ---- */
    CFCCL0 = CAN_CFG_C0_TRFIFO0_L; //未启用
    CFCCH0 = CAN_CFG_C0_TRFIFO0_H; //未启用
    ...
    C0CTRL = (C0CTRL & (CAN_STP_BIT_ON | CAN_MODE_BITS_ON)) + CAN_CFG_C0_FUNC_L; //开中断
...
Can_RtnType R_CAN_GlobalStart(void)
{
    ...
    if ((GSTS & CAN_GLB_RST_STS_BIT_ON) != 0U)
    {
        GCTRL = ((GCTRL & (uint16_t)~CAN_GLB_MODE_BITS_ON) |
                    CAN_GLB_OPERATION_MODE); // CAN_GLB_MODE_BITS_ON = 0 ； CAN_GLB_MODE_BITS_ON = 0  开中断
        if ((GSTS & CAN_GLB_RST_STS_BIT_ON) != 0U)
        {
            return CAN_RTN_MODE_WAIT;
        }
